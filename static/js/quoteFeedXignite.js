...
(function (definition) {
	...
	CIQ.QuoteFeed.Xignite.prototype.fetch=function(params, cb){
		...
		if(adjMethod && !isIndex && api && api.statics){
			getSplitInfo=false;
			api.statics=api.statics.replace("AdjustmentMethod=None","AdjustmentMethod="+adjMethod);
			if(params.stx.layout.adj){
				params.stx.layout.adj=false;
				if(params.stx.changeCallback) params.stx.changeCallback(params.stx, "layout");
			}
		}
	...
	CIQ.QuoteFeed.Xignite.getSnapshotQuote=function(params, symbol, isDaily, offset, cb, caller){
		...
		if(params.update &&
			...
			if(CIQ.Market.Symbology.isForexMetal(params.symbol)){
				CIQ.extend(api,{
					func: "xGlobalMetals", method: "GetRealTimeMetalQuote", fields: "Date,Time,Bid", statics: "Currency="
				});
			}else if(CIQ.Market.Symbology.isForexSymbol(params.symbol)){
				CIQ.extend(api,{
					func: "xGlobalCurrencies", method: "GetRealTimeRate", fields: "Date,Time,Bid"
				});
			}else{
				CIQ.extend(api,{
					func: "xBATSRealTime", method: "GetRealQuote", fields: "Date,Time,Last"
				});
				field="Last";
			}
