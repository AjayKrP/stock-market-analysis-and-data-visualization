...
(function (definition) {
			...
			if(styles.session){
				...
				for(var p in this.panels){
					...
					for(i=0;i<ranges.length;i++){
						...
						if(!noDashes && ranges[i].start>chart.left) this.plotLine(ranges[i].start, ranges[i].start, thisPanel.bottom, thisPanel.top, styles.divider.backgroundColor, "line", chart.context, thisPanel, {
							pattern: "dashed",
						...
						if(!noDashes && ranges[i].end<chart.right) this.plotLine(ranges[i].end, ranges[i].end, thisPanel.bottom, thisPanel.top, styles.divider.backgroundColor, "line", chart.context, thisPanel, {
							pattern: "dashed",
							lineWidth: dividerLineWidth
						});
					}
				...
				if((this.chart.dataSegment[bar][name] || this.chart.dataSegment[bar][name]===0) &&
					...
					if(this.chart.dataSegment[bar][name].constructor==Number){
						if(name=="Volume"){  // bad hack
							...
							//fieldValue=this.padOutPrice(this.chart.dataSegment[bar][name], 1.2345);
							fieldValue=this.formatPrice(this.chart.dataSegment[bar][name]);
						}
	...
	CIQ.Animation=function(stx, animationParameters, easeMachine){
		...
		stx.prepend("appendMasterData", function(appendQuotes, chart, params) {
			...
			if(chart.defaultChartStyleConfig.type==="line" || chart.defaultChartStyleConfig.type==="mountain" || chart.defaultChartStyleConfig.type==="baseline_delta") {
				...
				function cb(quote, prevQuote, chartJustAdvanced) {
					return function (newData) {
						...
						if (chartJustAdvanced) {
							...
							if (!q.High && q.High !== 0) q.High = q.Close;
							if (!q.Low && q.Low !== 0) q.Low = q.Close;
						}
						if (chart.animatingHorizontalScroll) {
				...
				var linearChart = !this.standaloneBars[this.layout.chartType];
	...
	CIQ.RangeSlider=function(params){
		...
		this.drawSlider=function(){
			...
			var chartSubholder=stx.chart.panel.subholder, ctx=self.chart.context, segmentImage=self.chart.segmentImage, halfCandle=self.layout.candleWidth/2;
			...
			var pLeft=self.pixelLeft=self.pixelFromTick(left)-(segmentImage?segmentImage[left].candleWidth/2:halfCandle);
			var pRight=self.pixelRight=self.pixelFromTick(right)+(segmentImage?segmentImage[right].candleWidth/2:halfCandle);
			ctx.save();
			ctx.beginPath();
