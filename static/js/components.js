		...
		if((doUpdatePrice || doUpdateBrowserTab) && symbol && priceChanged){
			...
			if(doUpdateBrowserTab){
				...
				if(todaysChangePct>0){
					document.title+="\u25b2 " + todaysChangeAbs;
...
<cq-themes>
	...
	TimezoneDialog.prototype.open=function(params){
		...
		if( stx.displayZone ) {
			...
			currentUserTimeZone.text('Current TimeZone is '+fullZone);
			button.show();
		} else {
			currentUserTimeZone.text('Your timezone is your current location');
			button.hide();
		}
